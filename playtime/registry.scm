    ...
    (methods
      ...
      ((register-role-player role-symbol role-instance)
        (let* ((player-symbol ($ role-instance 'symbol))
               (existing-instance (registry 'get-role-player-by-symbol role-symbol player-symbol))
               (current-players (hash-ref role-players role-symbol '()))
               (filtered-players (if existing-instance
                                   (filter (lambda (rp) (not (equal? ($ rp 'symbol) player-symbol)))
                                         current-players)
                                         current-players)))
          (hash-set! role-players role-symbol (cons role-instance filtered-players))
      ...
      ((get-role-player-by-symbol role-symbol player-symbol)
        (let ((all-players (hash-ref role-players role-symbol '())))
          (find (lambda (rp) (equal? ($ rp 'symbol) player-symbol))
                all-players)))
)))
